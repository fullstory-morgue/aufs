...
/* $Id: sysaufs.c,v 1.5 2007/04/30 05:43:57 sfjro Exp $ */
...
static struct super_block *find_sb_locked(struct kobject *kobj)
{
	...
	list_for_each_entry(sbinfo, &aufs_sbilist, si_list) {
		if (&sbinfo->si_sysaufs.subsys.kset.kobj != kobj)
			continue;
...
static void unreg(struct subsystem *subsys, struct sysaufs_entry *a, int n,
		  struct subsystem *parent)
{
	...
	for (i = 0; i < n; i++, a++)
		if (!a->err) {
			sysfs_remove_bin_file(&subsys->kset.kobj, &a->attr);
			if (Priv(a))
				free_entry(a);
		}
...
static int reg(struct subsystem *subsys, struct sysaufs_entry *a, int n,
	       struct subsystem *parent)
{
	int err, i;
	...
	subsys_get(parent);
	kset_set_kset_s(subsys, *parent);
	err = subsystem_register(subsys);
	if (unlikely(err))
		goto out;

	for (i = 0; !err && i < n; i++)
		err = a[i].err = sysfs_create_bin_file(&subsys->kset.kobj,
						       &a[i].attr);
	if (unlikely(err))
...
void sysaufs_add(struct aufs_sbinfo *sbinfo)
{
	...
	err = kobject_set_name(&sa->subsys.kset.kobj, "%p",
...

{
	...
	while (1) {
		...
		if (Allocated(e) > 0) {
			Allocated(e) <<= 1;
			if (unlikely(Allocated(e) >= PAGE_SIZE))
				Allocated(e) = 0;
		} else
			Allocated(e)--;
		//Dbg("%d\n", Allocated(e));
